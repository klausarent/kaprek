...
Before an agent turn runs a tool it can be made to ask. Those questions collect in `#/approvals` and wait there, so one raised by a trigger at 3am is still waiting when you open kaprek in the morning. You do not have to have been watching when it was asked.

Two deadlines bound the waiting, both fail-closed — a question nobody answers is **denied**, never allowed by default:
...
The waiting is not free. For as long as an unattended question sits there, that trigger's turn is still in flight: it holds a `claude` subprocess open, and `POST /api/triggers/<id>/fire` answers `429` for **every** trigger during that time — the manual-fire route refuses while any trigger-started turn is running (see [Known gaps](#known-gaps)). The chat that turn belongs to is locked the same way: sending it a message would destroy the very question the turn is parked on, so kaprek answers `409` until the question is answered or the turn ends. The eight-hour deadline bounds one question, not the turn: the agent may ask again the moment the first is denied, and what ends the turn then is its wall clock at **8h45m** (the deadline plus the turn's own remaining budget). That is the honest ceiling on the 429, and it is why the deadline is eight hours and not a day.
...
- **Approvals do not survive a restart.** The inbox (see [Approvals](#approvals)) makes a question outlive the browser tab that would have shown it. It cannot make one outlive kaprek itself: the CLI subprocess waiting on the answer dies with the server, so entries from a previous process are marked `process gone` on the next start and refused rather than answered. Restarting kaprek while a trigger waits on you loses that turn's question, and the trigger has to fire again.
- **A waiting question blocks manual trigger fires.** An unattended question keeps its turn in flight — up to 8h45m, the turn's wall clock, since one lapsed question does not stop the agent from asking the next — and for that whole time `POST /api/triggers/<id>/fire` answers `429` for every trigger, not just that one. Answering the question, or letting the turn end, clears it. The guarantee behind that refusal is narrower than it looks: kaprek runs at most one turn **per trigger** (its loop guard), and the manual-fire route additionally refuses while any trigger turn is running. Triggers that come due in the same tick do run concurrently, but no more than three at once.
- **Three parked questions block the fourth trigger.** A tick starts at most three trigger turns at once. It is a real ceiling, chosen because a parked question holds its `claude` subprocess for as long as it waits: ten triggers due at 3am would otherwise mean ten processes waiting until morning. A trigger that does not get a slot stays due and tries again on the next tick a minute later, and the log says it was deferred. A scheduled trigger whose window closes while all three slots are still parked misses that run outright.
- **Nobody has to be there.** The inbox removed the old "is anyone connected" check, which was never a presence test anyway (a forgotten tab in another window passed it). A `question` trigger now starts whether or not anyone will ever look. The failure direction is still safe — unanswered means denied — but the turn did run and did cost. What the caps see has caught up with that: a turn still parked on a question counts against both the trigger's daily runs and the global hourly ceiling from the moment it starts, not only once it ends (its cost counts only when the turn reports one).
