...
- **One server per data dir, enforced.** A second start against the same `<dataDir>` refuses to run rather than silently landing on another port. The lock is a socket, not a file: kaprek derives a port from the data dir path (49152 + hash) and binds it on `127.0.0.1`, and the OS refuses the second bind. A crashed instance releases its lock the moment the process dies, so there is no waiting period and nothing to clean up. If some unrelated program already sits on that port, kaprek asks it for a one-line greeting first and moves to the next port when the answer is not another kaprek on this data dir. See `src/lib/instance-lock.mjs`.
