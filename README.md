...
The waiting is not free. For as long as an unattended question sits there, that trigger's turn is still in flight: it holds a CLI subprocess open, and `POST /api/triggers/<id>/fire` answers `429` for **every** trigger during that time (kaprek allows at most one trigger-started turn at a time, deliberately — see [Known gaps](#known-gaps)). Eight hours of that is the price of an overnight question; it is why the deadline is eight hours and not a day.
...
- **A waiting question blocks the other triggers.** kaprek runs at most one trigger-started turn at a time (a deliberately coarse loop guard). An unattended question sitting in the inbox keeps its turn in flight for up to eight hours, and for that whole time `POST /api/triggers/<id>/fire` answers `429` for every trigger, not just that one. Answering the question, or letting it lapse, clears it.
