...
- **One server per data dir, enforced.** A second start against the same `<dataDir>` refuses to run rather than silently landing on another port. The lock is an OS handle, not a file: on Windows a named pipe whose name carries a hash of the data dir path, on macOS and Linux a TCP socket on `127.0.0.1` at a port derived from that same hash (23000–31999, clear of the ranges the OS hands out for outgoing connections). Either way the OS refuses the second bind, and a crashed instance releases its lock the moment the process dies — no waiting period, nothing left to clean up. If something else holds the address, kaprek asks it for a one-line greeting and steps aside to the next port when it gets an answer — either another kaprek naming a different data dir, or a service that plainly speaks some other protocol. What it does not do is step aside on silence: a timeout or a half-finished answer refuses the start after a few retries, because that is also what a running kaprek looks like while it is busy, and mistaking one for a stranger is how you end up with two. See `src/lib/instance-lock.mjs`.
