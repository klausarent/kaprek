 ...
 * through).
 */
async function handleApprovalDecision(req, res, id, pendingApprovals, approvalStore = null, getRunner = null, getRelay = null, grants = null) {
  if (!isSafeId(id)) {
  ...
  if (wantsGrant && typeof entry.inputHash === 'string' && approvalStore) {
    try {
      const decided = await approvalStore.decide(key, { ...decision, grantIntent: { inputHash: entry.inputHash } });
