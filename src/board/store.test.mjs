...
test('create adds a task with defaults and list/get return it', () => {
  ...
  const task = board.create({ title: 'Erster Task', project: 'ccview', tags: ['p1'] });
...
test('linkSession appends a session and dedupes on projectSlug+sessionId', () => {
  ...
  board.linkSession(task.id, { machine: 'klaus-pc', projectSlug: 'ccview', sessionId: 's1' });
  const after = board.linkSession(task.id, { machine: 'klaus-pc', projectSlug: 'ccview', sessionId: 's1' });

  expect(after.sessions).toEqual([{ machine: 'klaus-pc', projectSlug: 'ccview', sessionId: 's1' }]);

  const withSecond = board.linkSession(task.id, { projectSlug: 'ccview', sessionId: 's2' });
